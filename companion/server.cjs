...
function startRun(config) {
  ...
  const state = {run_id:id, status:'running', started_at:new Date().toISOString(), progress:[], report:null, error:null};
  runs.set(id, state);
  runConfig(config, {outputDir: require('path').join(process.cwd(), 'screenshots'), onProgress: event => state.progress.push({at:new Date().toISOString(), ...event})})
    .then(report => { state.status='completed'; state.report=report; state.completed_at=new Date().toISOString(); })
    .catch(error => { state.status='failed'; state.error=String(error.message || error); state.completed_at=new Date().toISOString(); });
  return id;
}
const server = http.createServer(async (req, res) => {
  ...
  if (match && req.method === 'GET') { const state=runs.get(match[1]); return state ? send(res,200,state,origin) : send(res,404,{ok:false,error:'Run not found'},origin); }
